...
def img_to_data_url(path: str) -> str:
    """
    Open an image file, convert to RGB PNG, and return a base64 data URL string
    that can be sent to Qwen as an image_url.
    """
    img = Image.open(path).convert("RGB")

    # Optional: resize huge images to something reasonable for speed
    max_side = 2000
    w, h = img.size
    if max(w, h) > max_side:
...
def _parse_qwen_json(raw_content: str) -> Dict[str, Any]:
    """
    Qwen *should* return pure JSON, but sometimes models wrap it in fences or extra text.
    This tries to robustly extract the JSON object.

    Returns a dict. If parsing fails, returns a default 'illegible' structure.
    """
    if not raw_content:
        return {
            "medications": [],
            "illegible": True,
            "illegible_details": "Empty OCR response from model.",
        }

    # Remove common markdown fences if they appear
    cleaned = raw_content.strip()
    if cleaned.startswith("```"):
        # Remove ```json or ``` and closing ```
        cleaned = cleaned.strip("`")
        # Sometimes it's like "json{...}" → remove leading 'json'
    ...
    try:
        ...
        try:
            ...
            # Final fallback → mark as illegible
            return {
                "medications": [],
                "illegible": True,
                "illegible_details": f"Failed to parse OCR JSON: {e}",
            }


def call_qwen_model(image_path: str) -> Dict[str, Any]:
    """
    Call Qwen multimodal model with the prescription image and the extraction prompt.

    Returns:
        A dict with at least:
          - medications: list[dict]
          - illegible: bool
          - illegible_details: str | None
    """
    ...
    result = _parse_qwen_json(raw_content)

    # Ensure required keys exist even if the model is slightly off
    if "medications" not in result or not isinstance(result.get("medications"), list):
        result["medications"] = []

    if "illegible" not in result:
        result["illegible"] = len(result["medications"]) == 0

    if "illegible_details" not in result:
        result["illegible_details"] = None
